...
class KNN:
    ...
    def euclid(self, inputs, method):
        """ユークリッド距離"""
        self._X["Euclid"] = (((self.X - inputs)**2).sum(axis=1))**0.5
        self.predict_to_similality(method)
    
    def manhattan(self, inputs, method):
        """マンハッタン距離"""
        self._X[method] = abs(self.X - inputs)
        self.predict_to_similality(method)
    
    def mahalanobis(self, inputs, method):
        """マハラノビス距離"""
    ...
    def chebyshev(self, inputs, method):
        """チェビシェフ距離"""
        self._X[method] = np.max((self.X - inputs), axis=1)
        self.predict_to_similality(method)
    
    def predict_to_similality(self, method):
        """類似度を基に予測結果を計算する"""
    ...
    def predict(self, input_data, method="Euclid"):
        ...
        if method=="Euclid":
            input_data.apply(lambda x : self.euclid(x, method), axis=1)
        elif method=="Manhattan":
            input_data.apply(lambda x : self.manhattan(x, method), axis=1)
        elif method=="Chebyshev":
            input_data.apply(lambda x : self.chebyshev(x, method), axis=1)
        else:
            self.cov_inv = np.linalg.pinv(self.X.cov())
            input_data.apply(lambda x : self.mahalanobis(x, method), axis=1)
